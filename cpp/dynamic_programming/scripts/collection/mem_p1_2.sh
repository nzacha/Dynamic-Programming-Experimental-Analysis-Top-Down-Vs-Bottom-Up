...
do
	...
	do
		...
		do
			...
			valgrind --tool=massif --main-stacksize=15032385536 --massif-out-file=mem_results/${problem}/massif${size}_${attempt}.txt --stacks=yes ./executable.out ${problem} ${size} -iterative -q
...
do
	...
	do
		...
		do
			...
			valgrind --tool=massif --main-stacksize=15032385536 --massif-out-file=mem_results/${problem}/massif${size}_${attempt}.txt --stacks=yes ./executable.out ${problem} ${size} -recursive -q
