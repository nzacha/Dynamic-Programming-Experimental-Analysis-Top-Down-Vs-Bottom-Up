...
void* findMCSS_recursive(char* seqA, char* seqB){
	int height = strlen(seqA);
	int width = strlen(seqB);
...
void* printSolution_recursive(char* seqA, char* seqB){
	#ifdef DEBUG
	printf("\nPrinting solution for \"%s\" and \"%s\"\n", seqA, seqB);
	#endif
	...
	for(int i = height; i >= 0 && j >= 0;){
		for(; i >= 0 && j >= 0;){
			if((lookup(t, i-1, j-1) == (lookup(t, i, j) -1)) && (lookup(t, i, j-1) < lookup(t, i, j)) && lookup(t, i-1, j) < lookup(t, i, j)){				
				...
				i--;
			}else if (lookup(t, i, j) == lookup(t, i, j-1)){
				j--;
			}
		}
	}
...
int main(int argc, char** argv){	
	...
	if(method == 1){
		...
		time = clock();
		findMCSS_recursive(seqA, seqB);		
		time = clock() - time;

		#ifdef DEBUG
			printHashMap(t);
		#endif
		printSolution_recursive(seqA, seqB);
	...
	printf("NOTE: 2^16 is the limit for new algorithm of top-down since row, col are unsigned short\n");
